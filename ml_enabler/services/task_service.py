...
class TaskService:
    ...
    def create(payload: dict) -> int:
        ...
        task.create(
            payload.get("pred_id"), payload.get("type"), payload.get("batch_id")
        )
    ...
    def delete(task_id: int):
        ...
        if task:
            ...
            if task.batch_id:
                ...
                batch.cancel_job(jobId=task.batch_id, reason="User Requested")
    ...
    def list(pred_id: int, task_type):
        ...
        if rawtasks:
            ...
            return {"pred_id": pred_id, "tasks": tasks}

        return {"pred_id": pred_id, "tasks": []}
    ...
    def get(task_id: int):
        ...
        if task.get("batch_id") is not None:
            ...
            desc = batch.describe_jobs(jobs=[task.get("batch_id")])
            ...
            if len(desc.get("jobs")) == 1:
                ...
                if (
                    ...
                    task["logs"] = desc["jobs"][0]["attempts"][0]["container"][
                        "logStreamName"
                    ]
                else:
                    task["logs"] = False
